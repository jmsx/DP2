...
<display:table pagesize="10" class="displayspammer" keepStatus="true"
	name="actors" requestURI="ban/administrator/listNotBannedSpammer.do" id="row">
	
	<!-- Action links -->

<security:authorize access="hasRole('ADMIN')">
		
		<spring:message code="actor.ban.header" var="banHeader"></spring:message>
		<display:column title="${banHeader}" sortable="false">
			<spring:url value="ban/administrator/ban.do" var="banURL">
				<spring:param name="actorId" value="${row.id}"/> <!-- es el parámetro que va a acompañar a la url -->
			</spring:url>
			<a href="${unbanURL}"><spring:message code="ban.edit"/></a>
		</display:column>	
		</security:authorize>		
