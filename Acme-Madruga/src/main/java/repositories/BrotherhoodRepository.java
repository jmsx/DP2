...
public interface BrotherhoodRepository extends JpaRepository<Brotherhood, Integer> {
	...
	@Query(value = "SELECT STDDEV(x),MAX(x),MIN(x),AVG(x) FROM (SELECT COUNT(*) AS x FROM `acme-madruga`.ENROLMENT GROUP BY brotherhood) AS x", nativeQuery = true)
	Double[] getStatisticsOfMembersPerBrotherhood();

	/** The largest brotherhood is the one with highest number of members **/
	@Query(
		value = "SELECT ENROLMENT.brotherhood FROM `acme-madruga`.ENROLMENT LEFT OUTER JOIN BROTHERHOOD ON ENROLMENT.id=BROTHERHOOD.id GROUP BY brotherhood HAVING COUNT(*) = (SELECT MAX(x) FROM (SELECT COUNT(*) AS x FROM `acme-madruga`.ENROLMENT GROUP BY brotherhood)AS T)",
		nativeQuery = true)
	Integer[] getLargestBrotherhood();

	/** The smallest brotherhood is the one with lowest number of members **/
	@Query(
		value = "SELECT ENROLMENT.brotherhood FROM `acme-madruga`.ENROLMENT LEFT OUTER JOIN BROTHERHOOD ON ENROLMENT.id=BROTHERHOOD.id GROUP BY brotherhood HAVING COUNT(*) = (SELECT MIN(x) FROM (SELECT COUNT(*) AS x FROM `acme-madruga`.ENROLMENT GROUP BY brotherhood)AS T)",
		nativeQuery = true)
